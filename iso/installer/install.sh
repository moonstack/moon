...
cp /root/installer/sources.list /etc/apt/sources.list
...
mySITES="https://gitee.com https://pypi.python.org https://mirrors.aliyun.com/ubuntu https://get.daocloud.io"
...
# Install types are MOON, HP, INDUSTRIAL, ALL
tput cnorm
...
# Installing docker-compose, wetty, ctop, elasticdump, moon, daocloud
...
npm install https://gitee.com/stackw0rm/wetty.git -g 2>&1 | dialog --title "[ Installing wetty ]" $myPROGRESSBOXCONF
...
addgroup --gid 2000 moon 2>&1 | dialog --title "[ Adding new user ]" $myPROGRESSBOXCONF
adduser --system --no-create-home --uid 2000 --disabled-password --disabled-login --gid 2000 moon 2>&1 | dialog --title "[ Adding new user ]" $myPROGRESSBOXCONF
...
case $myFLAVOR in
  HP)
    ...
    cp /opt/moon/etc/compose/hp.yml $myMOONCOMPOSE 2>&1>/dev/null
  ;;
  INDUSTRIAL)
    echo "### Preparing INDUSTRIAL flavor installation."
    cp /opt/moon/etc/compose/industrial.yml $myMOONCOMPOSE 2>&1>/dev/null
  ;;
  MOON)
    echo "### Preparing MOON flavor installation."
    cp /opt/moon/etc/compose/moon.yml $myMOONCOMPOSE 2>&1>/dev/null
  ;;
  EVERYTHING)
    echo "### Preparing EVERYTHING flavor installation."
    cp /opt/moon/etc/compose/all.yml $myMOONCOMPOSE 2>&1>/dev/null
  ;;
esac

# Let's load docker images
myIMAGESCOUNT=$(cat $myMOONCOMPOSE | grep -v '#' | grep image | cut -d: -f2 | wc -l)
j=0
for name in $(cat $myMOONCOMPOSE | grep -v '#' | grep image | cut -d'"' -f2)
  do
...
cp -R /opt/moon/host/etc/nginx/ssl /etc/nginx/ 2>&1 | dialog --title "[ Copy configs ]" $myPROGRESSBOXCONF
cp    /opt/moon/host/etc/nginx/moonweb.conf /etc/nginx/sites-available/ 2>&1 | dialog --title "[ Copy configs ]" $myPROGRESSBOXCONF
cp    /opt/moon/host/etc/nginx/nginx.conf /etc/nginx/nginx.conf 2>&1 | dialog --title "[ Copy configs ]" $myPROGRESSBOXCONF
cp    /opt/moon/host/usr/share/nginx/html/* /usr/share/nginx/html/ 2>&1 | dialog --title "[ Copy configs ]" $myPROGRESSBOXCONF
cp    /root/installer/keys/authorized_keys /home/msec/.ssh/authorized_keys 2>&1 | dialog --title "[ Copy configs ]" $myPROGRESSBOXCONF
...
# Let's enable MoonStack website
ln -s /etc/nginx/sites-available/moonweb.conf /etc/nginx/sites-enabled/moonweb.conf 2>&1 | dialog --title "[ Enabling MoonStack website ]" $myPROGRESSBOXCONF
...
chmod 600 /home/msec/.ssh/authorized_keys 2>&1 | dialog --title "[ Set permissions and ownerships ]" $myPROGRESSBOXCONF
chown msec:msec /home/msec/.ssh /home/msec/.ssh/authorized_keys 2>&1 | dialog --title "[ Set permissions and ownerships ]" $myPROGRESSBOXCONF
